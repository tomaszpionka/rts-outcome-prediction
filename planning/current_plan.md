...
**A-17 (PR #281 lineage refresh note; forward-pointing; binding for PR #281's next amendment cycle):** After this PR #282 amendment merges, PR #281's `planning/INDEX.md` Active row stays as the active Layer-2 plan row. PR #281's NEXT amendment refresh MUST archive THIS amendment PR (#282) in `planning/INDEX.md` as a NEW archive row alongside the three existing archive rows (PR #278 V3 scaffold, PR #279 Layer-1 plan, PR #280 Round 1 amendment). The PR #281 INDEX archive section should therefore grow from 3 rows to 4 rows in PR #281's amendment refresh. This PR (#282) MUST NOT itself touch `planning/INDEX.md` — the active row is owned by PR #281, and self-archive of #282 in its own PR would violate A-16. The PR #281 executor performing the amendment refresh is the unique author of this lineage update.
...
- **H7 (Q8 syntactic-only guard; from V3 plan; NIT-A6: extended to include .ipynb):**
  ...
  Empirical-claim raw predicate: `grep -niE 'aoe2.*transferab|transferab.*aoe2|validated on aoe2|aoe2.*verified|cross-game validated' <target>`. Narrowed predicate (binding from PR #282 NIT-2; NIT-X2 `\b` boundaries applied for defense-in-depth against adversarial substring smuggling): pipe the raw output through `grep -vE '^[^:]+:[0-9]+:\s*[A-Z][A-Z0-9_]*\s*=|\bH7_FORBIDDEN\b|\bH7_REGEX\b|\bFORBIDDEN_AOE2_CLAIM\b|\bSYNTACTIC_ONLY\b|NOT empirical|no empirical|deferred to .*AoE2|Q8 stance|syntactic[-_ ]only|^[^:]+:[0-9]+:\s*#.*(sentinel|Q8.*(syntactic|sentinel|family[-_ ]kind))'` then `wc -l` — the final count across all 5 targets MUST equal 0. The narrowed predicate carves out sentinel constants (e.g., `FAMILY_KIND_Q8 = "q8_aoe2_transferability"`), self-referential regex definitions (`H7_FORBIDDEN`, `H7_REGEX`, `FORBIDDEN_AOE2_CLAIM` constants and their test parametrisations — with `\b` boundaries to prevent substring smuggling like `H7_FORBIDDEN_aoe2_transferability_is_proven`), explicit anti-claims (lines containing `SYNTACTIC_ONLY` as a bounded token, or `NOT empirical`, `no empirical`, `deferred to … AoE2`, `Q8 stance`, `syntactic-only`, `syntactic_only`), and sentinel comment markers. The narrowed predicate still catches empirical claims like `"validated on aoe2 data confirms transferability"` (no sentinel/anti-claim marker) and `"aoe2 verified successful cross-game"` (no carve-out keyword). No empirical AoE2 transferability claim permitted in adjudicator source, test, notebook (.py and .ipynb), or decision MD. AoE2 transferability deferred to future AoE2-specific Phase 02 step (syntactic-only structural reusability is acceptable but cannot be empirically claimed).
...
Adjudicator main entrypoint: `adjudicate_temporal_feature_grid(repo_root: Path) -> AdjudicationResult` (matches the module file name `adjudicate_temporal_feature_grid.py` and the `adjudicate_*.py` sibling precedent in the same directory — see NOTE-3 below).

**NOTE-3 (pre-execution gate binding; carried forward into PR #281 final gate; resolved in PR #282 amendment):** The public entrypoint name `adjudicate_temporal_feature_grid(repo_root: Path) -> AdjudicationResult` was selected because it (a) matches the module file name `src/rts_predict/games/sc2/datasets/sc2egset/adjudicate_temporal_feature_grid.py`, and (b) follows the `adjudicate_*.py` precedent established by the sibling decision-record modules in the same directory: `adjudicate_history_cross_region_retention.py`, `adjudicate_history_enriched_pre_game_source_layer.py`, `adjudicate_history_rating_reconstruction.py`, `adjudicate_pre_game_source_layer.py`, and `adjudicate_symmetry_difference_feature_scope.py`. The pre-execution reviewer-adversarial gate confirmed this naming choice as binding prior to Layer-2 dispatch; the prior plan text at line 538 + the NIT-A3 audit row at line 556 used the stale form `validate_temporal_feature_grid_adjudication` (verb–noun-inverted, prefix-incongruent with the module file). PR #282 amendment corrects both occurrences and lands this NOTE-3 paragraph as the binding record of the rationale.
