...
- **Amendment 2 of PR #208** (`src/rts_predict/games/sc2/datasets/sc2egset/reports/INVARIANTS.md:97`): *"Tracker-derived features are never pre-game [...] every row in `tracker_events_feature_eligibility.csv` carries `status_pre_game = not_applicable_to_pre_game`"*. The new §4.3.3 MUST state this verbatim or near-verbatim in Polish.
...
| 4 | Tracker-derived SC2 features are NEVER pre-game (Invariant I3 + Amendment 2 per PR #208) | `.claude/scientific-invariants.md` I3; `src/rts_predict/games/sc2/datasets/sc2egset/reports/INVARIANTS.md:97`; `tracker_events_feature_eligibility.csv` 15/15 rows with `status_pre_game = not_applicable_to_pre_game` | new §4.3.3 — must be stated verbatim or near-verbatim, and explicitly cited to Invariant I3 + Amendment 2. |
