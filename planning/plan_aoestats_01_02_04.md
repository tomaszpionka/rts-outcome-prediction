...
- `match_rating_diff`: rating change (DOUBLE)
...
For all 18 `matches_raw` columns plus all 14 `players_raw` columns, compute cardinality. Flag any column with cardinality = 1 (constant, per EDA Manual Section 3.3) or uniqueness ratio < 0.001 (near-constant, threshold from EDA Manual Section 3.3).
...
2. Append a step definition block for `01_02_04` after the `01_02_03` block, using the same YAML schema.
   ...
   - `question`: "What are the NULL rates, value distributions, cardinality, and descriptive statistics across all columns in matches_raw and players_raw, what is the class balance of the target variable (winner), and what fraction of data is 1v1?"
...
2. Section 1: Full NULL census of `matches_raw` — all 18 columns with count and percentage (Section A SQL). Pull to pandas `.df()`, reshape into tidy `(column, null_count, null_pct)` table.
3. Section 2: Full NULL census of `players_raw` — all 14 columns (Section B SQL). Same tidy-table treatment.
...
5. Section 4: Player-count distribution — `num_players` from `matches_raw` (Section D SQL). This establishes thesis scope.
...
10. `research_log.md` entry mentions deferred `overviews_raw` unnesting and `new_rating` leakage note.
...
**Steps 01_02_02 (DuckDB Ingestion) and 01_02_03 (Raw Schema DESCRIBE) must be complete** before executing this plan. The persistent DuckDB exists on disk (created 2026-04-13). The 01_02_03 DESCRIBE notebook ran successfully against it, producing schema YAMLs for all three tables. Note: STEP_STATUS.yaml currently shows 01_02_02 as `not_started` and does not list 01_02_03 — the status file is stale. The de facto state is that both steps are complete (DuckDB on disk, 01_02_03 artifacts and research log entry on disk). T02 of this plan updates STEP_STATUS for 01_02_04 only; bringing 01_02_02 and 01_02_03 status up to date is a separate housekeeping task.
...
**STEP_STATUS.yaml is stale.** It shows 01_02_02 as `not_started` and does not list 01_02_03 at all, despite both steps being de facto complete. This plan does not fix that — it is a separate housekeeping concern.
