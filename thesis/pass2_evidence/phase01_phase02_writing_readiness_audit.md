...
## 10. Existing draft conformance audit

This section compares the existing draft thesis prose under `thesis/chapters/01_introduction.md` through `thesis/chapters/07_conclusions.md` against current repository evidence after PR #216 (SC2EGSet provisional feature-family registry artifact, `validated_through = V-9`) merged to master. Source for the underlying per-claim classification: T05 read-only scratch deliverable executed on 2026-05-17 against branch HEAD `aa718f88` (planner-science, Opus). The section is itself Pass-2 evidence; it does not modify any chapter file and does not draft chapter prose. The actionable routing of every row in §10.6 lands in §11 (Draft correction backlog) and §12 (Writing agent task queue) below.

Classification key (per the amended `planning/current_plan.md` §T05–§T07):

- **C1 Safe-and-supported** — drafted claim aligns with current evidence; no edit required.
- **C2 Stale / numerically outdated** — drafted claim is contradicted by evidence that has landed since the prose was written.
- **C3 Overclaim** — drafted claim asserts a closure / completeness / result not justified by evidence (cross-checked against §8 Forbidden Claims F1–F18).
- **C4 Claims requiring caveats** — drafted claim is materially correct but missing a binding qualifier.
- **C5 Missing claims to add** — current evidence supports a claim that no chapter currently makes.
- **C6 Exact proposed edits** — per-row routing artifact produced from §10.2–§10.5; consumed by §11 + §12.

### 10.1 Safe-and-supported claims

The following chapter loci hold up against current evidence as of `aa718f88`. They require no edit in this PR and no edit in any future writing PR scoped to PR #216 evidence; reviewer routing is the lightweight `reviewer` agent. Grouped per drafted subsection; each bullet states the claim summary, the evidence artifact, and the status.

- **`thesis/chapters/01_introduction.md` §1.3 RQ3 and §1.4 bounded comparative framing.** Claim: cross-game differences between SC2 and AoE2 are a confounded mixture of mechanics, data regime, source population, and feature availability. Evidence: `thesis/pass2_evidence/cross_dataset_comparability_matrix.md` (five-axis bounded comparability); `thesis/pass2_evidence/methodology_risk_register.md` RISK-05/06. Status: C1, no caveat missing; routes to `reviewer`.
- **`thesis/chapters/01_introduction.md` §1.4 AoE2 source-label discipline.** Claim: aoestats `leaderboard='random_map'` is Tier 4 with queue-semantics unverified; aoe2companion combines `internalLeaderboardId=6` (ranked candidate) and `internalLeaderboardId=18` (quickplay/matchmaking-derived) and the union must not be called "ranked ladder" without explicit qualification. Evidence: `thesis/pass2_evidence/aoe2_ladder_provenance_audit.md` §4 (four-tier governance); RISK-01/02/03/04. Status: C1; routes to `reviewer`.
- **`thesis/chapters/02_theoretical_background.md` §2.2.2 SC2EGSet date range.** Claim: corpus spans 2016–2024. Evidence: §3.1 + §3.2 of this audit (verified against `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/01_inventory/01_01_01_file_inventory.{json,md}` and 01_02_04 univariate census). Status: C1; routes to `reviewer`.
- **`thesis/chapters/02_theoretical_background.md` §2.2.3 SC2 tournament-vs-ladder framing.** Claim: SC2EGSet is a tournament-replay corpus, not a Battle.net ladder corpus; `leaderboard_raw` is NULL for 100% of rows. Evidence: §3.4 of this audit (forbidden claim F17 correctly avoided). Status: C1; routes to `reviewer`.
- **`thesis/chapters/02_theoretical_background.md` §2.2.4 SC2 engine constants.** Claim: 22.4 game loops per real-time second; PlayerStats events emit on a 160-loop period. Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_04_event_profiling.{json,md}`. Status: C1; routes to `reviewer`.
- **`thesis/chapters/03_related_work.md` §3.5 luka-3 narrowing.** Claim: Lin et al. 2024 use AoE2 to validate a balance-analysis method, which is a different task from outcome prediction. Evidence: `thesis/pass2_evidence/literature_verification_log.md`; audit §6.5 / §3.4 row 9. Status: C1; routes to `reviewer`.
- **`thesis/chapters/04_data_and_methodology.md` §4.1.1 SC2EGSet data description.** Claim: 70 tournament directories, 22 390 `.SC2Replay.json` files, ~214 GB cumulative. Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/01_inventory/01_01_01_file_inventory.{json,md}`. Status: C1; routes to `reviewer-deep` for any Phase-03 numerical revision.
- **`thesis/chapters/04_data_and_methodology.md` §4.1.1.4 MMR taxonomy.** Claim: MMR=0 covers 83.95% of rows in `matches_flat_clean`; ledger classifies the column as MAR-primary with MNAR-sensitivity branch per Rubin1976 taxonomy. Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/04_cleaning/01_04_01_missingness_ledger.csv`. Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.1.2.1 aoestats source label and side asymmetry.** Claim: aoestats labelled Tier 4 with semantic opacity; side asymmetry `team=1 win=52.27%` flagged under `[PRE-canonical_slot]`. Evidence: `src/rts_predict/games/aoe2/datasets/aoestats/reports/artifacts/01_exploration/04_cleaning/01_04_00_source_normalization.{json,md}`; `thesis/pass2_evidence/aoe2_ladder_provenance_audit.md`. Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.1.2.2 aoe2companion mixed-mode side asymmetry.** Claim: `team1_wins=47.18%`; `team2_wins=52.82%`. Evidence: `src/rts_predict/games/aoe2/datasets/aoe2companion/reports/artifacts/01_exploration/04_cleaning/01_04_00_source_normalization.{json,md}`. Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.1.3 / §4.1.4 five-axis bounded comparability framing.** Claim: cross-game comparison is bounded per Tabela 4.4a/4.4b; `[POP:]` tag inventory is dataset-conditional. Evidence: `thesis/pass2_evidence/cross_dataset_comparability_matrix.md`. Status: C1 for framing; see §10.4 for the aoestats interface-CSV row-count caveat.
- **`thesis/chapters/04_data_and_methodology.md` §4.1.3 patch-66692 reference-window argument.** Claim: patch 66692 is the only patch fully covering the pre-registration observation window [2022-08-29, 2022-10-27] (123 367 of 241 981 matches inside window). Evidence: `reports/specs/01_05_preregistration.md` (lines 580–582 cited inline in the chapter). Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.2.1 ingestion / schema methodology.** Claim: two-path ingestion preserves filename relative to `raw_dir` per Invariant I10. Evidence: `.claude/scientific-invariants.md` (I10); per-dataset `INVARIANTS.md` §2. Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.2.2 identity-resolution methodology.** Claim: I2 5-branch identity resolution (sc2egset Branch iii / aoestats Branch v / aoe2companion Branch i) with migration_rate and collision_rate measures. Evidence: per-dataset `INVARIANTS.md` §2; `thesis/pass2_evidence/sec_4_2_crosswalk.md`. Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.2.3 cleaning typology.** Claim: Tabela 4.6 mechanism × band × action structure; MissingIndicator defence anchored in Jakobsen2017, vanBuuren2018, SchaferGraham2002, MadleyDowd2019. Evidence: `thesis/pass2_evidence/cleanup_flag_ledger.md`; audit §3.4. Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.4.4 within-game and cross-game statistical-comparison framing.** Claim: within-game protocol deferred to §4.4.2; cross-game omnibus Friedman test inapplicable at N=2 per Demsar2006 §3.2 (N ≥ 10 requirement). Evidence: audit §6.8; F15 correctly avoided. Status: C1; routes to `reviewer-deep`.
- **`thesis/chapters/04_data_and_methodology.md` §4.4.5 ICC / ANOVA estimator.** Claim: Tabela 4.7 reports per-dataset ICC ANOVA values with estimator-choice defence (sc2egset 0.0463; aoestats 0.0268; aoe2companion 0.003013). Evidence: `src/rts_predict/games/<game>/datasets/<dataset>/reports/artifacts/01_exploration/05_panel_design/01_05_05_icc_*.{json,csv,md}`. Status: C1 for ANOVA numbers; sc2egset CI-method cell already carries an `UNVERIFIED`-prefixed inline flag in the chapter — see §10.4 row 7.
- **`thesis/chapters/04_data_and_methodology.md` §4.4.6 `canonical_slot` / `[PRE-canonical_slot]` flag scope.** Claim: the flag distinguishes raw-schema `p0_civ` / `p1_civ` from post-Phase-02 UNION-ALL aggregate; per `reports/specs/02_00_feature_input_contract.md` §1 W3 ARTEFACT_EDGE. Evidence: spec §1 line 71. Status: C1; routes to `reviewer-deep`.

### 10.2 Stale / numerically outdated claims

The following drafted claims are contradicted by evidence that has landed since the prose was written. Each row provides the file, the section / line range, the current claim (verbatim where actionable), the evidence source, and the recommended correction direction. Row 3 (aoestats interface-CSV row count) is classified as C4 caveat in §10.4 rather than C2 stale; it is cross-referenced here for completeness.

- **C2.1 — `thesis/chapters/04_data_and_methodology.md` §4.3.2 (line ~331).** Current claim (verbatim): *"Do czasu wykonania odrębnej walidacji semantycznej Step 01_03_05 — która nie została zrealizowana w obecnej iteracji — cechy oparte bezpośrednio na `tracker_events_raw` nie są traktowane jako w pełni zwalidowany komponent wejściowy modeli..."*. Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/STEP_STATUS.yaml` records Step 01_03_05 with `status: complete` and `completed_at: 2026-05-05`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md` reports GATE-14A6 outcome `narrowed` (not closed); `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv` enumerates 5 `eligible_for_phase02_now` + 7 `eligible_with_caveat` + 3 `blocked_until_additional_validation`; `thesis/pass2_evidence/phase02_readiness_hardening.md` §14A.6. Recommended correction direction: rewrite the paragraph to acknowledge that Step 01_03_05 is complete; that GATE-14A6 is `narrowed` (not closed); that 12 planned-yes tracker rows are available (5 immediate + 7 with caveat) and 3 remain blocked; that tracker-derived features are NEVER pre-game (Invariant I3, Amendment 2 per PR #208). Severity: **HIGH**. Future agent: `planner-science-then-writer-thesis` (methodology design must precede prose). Reviewer routing: `reviewer-adversarial` (GATE-14A6 framing is methodology-load-bearing).
- **C2.2 — `thesis/chapters/02_theoretical_background.md` §2.5.5 (line ~179) ⨯ `thesis/chapters/03_related_work.md` §3.2.4 (line ~77) ⨯ `thesis/chapters/03_related_work.md` §3.5 (line ~189).** Cross-locus EsportsBench version-and-cutoff drift. §2.5.5 cites `v8.0, cutoff 2025-12-31`; §3.2.4 cites `v9.0, cutoff 2025-09-30; v8.0 planowana z cutoff 2025-12-31` (internally inconsistent — `v9.0` paired with a `v7.0`-era cutoff and contradicted by `v8.0 planowana`); §3.5 cites `v9.0 (cutoff 2026-03-31, weryfikacja karty zbioru danych HuggingFace 2026-04-26)`. Evidence: `thesis/pass2_evidence/literature_verification_log.md`; `thesis/WRITING_STATUS.md` chap2 / chap3 rows (F6.6 closed locally after HF commit verification). Recommended correction direction: pick one canonical (version, cutoff, verification-date) triple at thesis Pass-2 finalization and harmonize all three loci to it; remove the §3.2.4 internal contradiction first. Severity: **MEDIUM**. Future agent: `writer-thesis` (Pass-2 reconciliation; no methodology design needed). Reviewer routing: `reviewer-deep`.
- **C2.3 — `thesis/chapters/04_data_and_methodology.md` §4.1.4 (line ~212).** See §10.4 row 1 below — classified as C4 caveat (header-vs-data-row framing of `phase06_interface_aoestats.csv` row count). Cross-referenced here so any future correction PR that touches §4.1.4 finds both the §10.2 stale-cross-reference and the §10.4 caveat row at one place.

### 10.3 Overclaims

**C3 overclaim count: 0.**

No drafted chapter prose currently claims any of the following (all of which are forbidden per §8 of this audit and per `.claude/rules/data-analysis-lineage.md`):

- Phase 02 closure for any dataset (F4 holds).
- Step 02_01_01 closure for SC2EGSet (F4 holds; PR #216 emitted a provisional artifact at `validated_through = V-9` without claiming Step closure).
- Final feature catalog for SC2EGSet, aoestats, or aoe2companion (F3 holds).
- Leakage-free materialized features (F7 holds; CROSS-02-01-v1.0.1 post-materialization audit remains mandatory).
- Model-ready feature matrix (F3 holds; no Phase 02 materialization has occurred).
- Tabular-vs-GNN conclusion (F2 holds; no model results exist).
- Cross-game generalizability beyond the bounded four-confound framing of §1.3 RQ3 / §1.4 / Tabela 4.4a/4.4b (F1 / F5 hold; chapter prose carries the four-confound disclaimer where required).
- AoE2 source-label discipline violation (aoestats "ranked ladder" unqualified; aoe2companion `ID 6` and `ID 18` conflated; combined union mislabelled "ranked-only"): no occurrences found in drafted chapter prose; T11/T12/T13 sweeps have held.

Placeholder HTML comments in `thesis/chapters/05_experiments_and_results.md`, `thesis/chapters/06_discussion.md`, and `thesis/chapters/07_conclusions.md` are recorded as **future-PR risk anchors** (cross-listed as C5 missing-claim rows in §10.5) rather than current overclaims, per the Q1 resolution recorded in `planning/current_plan.md` §Open questions.

### 10.4 Claims requiring caveats

The following claims are materially correct but require a binding qualifier whenever they are touched by a future writing PR or extended by a new subsection. The first row is a current chapter caveat; rows 2–7 are forward-looking caveats that must be embedded by the writer when the corresponding §10.5 missing-claim row is drafted.

1. **`thesis/chapters/04_data_and_methodology.md` §4.1.4 (line ~212) — aoestats interface-CSV row-count framing.** Chapter prose says `phase06_interface_aoestats.csv` carries 136 rows with an aoestats `[POP:]` tag; the audit elsewhere reports 137 rows with 0 of 137 `[POP:]` tags (scope is implicit via spec §0 + R02 rather than tag-carried). Recommended caveat at next touch: clarify whether the chapter is counting 137 rows or 136 data rows (post-header) and acknowledge that aoestats scope is **implicit-via-spec-§0** rather than **tag-present**. Severity: **MEDIUM**. Future agent: `writer-thesis`. Reviewer routing: `reviewer-deep`.
2. **NEW `thesis/chapters/04_data_and_methodology.md` §4.5 (Phase 02 registry methodology — does not yet exist).** Required caveat at draft time: the SC2EGSet feature-family registry is **provisional**, `validated_through = V-9`, and the manifest status token is **`partial_coverage_v9_baseline`**. Step 02_01_01 closure is NOT claimed. Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.md`; `thesis/pass2_evidence/notebook_regeneration_manifest.md`. Severity: **MEDIUM**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`.
3. **NEW §4.5 — CROSS-02-01-v1.0.1 mandatory post-materialization audit.** Required caveat: the registry's `validated_through = V-9` status does NOT excuse any future materialized feature column from CROSS-02-01-v1.0.1 leakage audit. Evidence: `reports/specs/02_01_leakage_audit_protocol.md` (CROSS-02-01-v1.0.1, LOCKED 2026-05-03). Severity: **MEDIUM**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`.
4. **NEW §4.5 — per-dataset Phase 02 asymmetry.** Required caveat: aoestats and aoe2companion Phase 02 = ROADMAP-stub only; no provisional registry artifact exists for either dataset; any Phase-02-fed claim in the thesis MUST be qualified by dataset. Evidence: aoestats + aoe2companion `PHASE_STATUS.yaml` Phase 02 entries (not started); §5.2 of this audit. Severity: **HIGH**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`.
5. **NEW §4.3.x SC2 tracker eligibility (does not yet exist).** Required caveat: GATE-14A6 outcome is `narrowed` (not closed). The 12 planned-yes tracker rows are available under per-row `eligibility_scope` and `caveat` constraints; the 3 blocked rows remain excluded until a future dedicated validation Step resolves the blocking reason; tracker-derived SC2 features are **never pre-game** (Invariant I3 / Amendment 2). Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`. Severity: **HIGH**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`.
6. **AoE2 source-label discipline.** Required caveat at any future touch of §1.4, §2.2.3, §4.1.2.1, §4.1.2.2, §4.1.3: aoestats Tier 4 (`leaderboard='random_map'` with unverified queue semantics), aoe2companion `ID 6` (ranked candidate) vs `ID 18` (quickplay/matchmaking-derived), combined `ID 6 + ID 18` is mixed-mode not ranked-only. Evidence: `thesis/pass2_evidence/aoe2_ladder_provenance_audit.md`; `thesis/pass2_evidence/methodology_risk_register.md` RISK-01/02/03/04. Severity: **MEDIUM**. Future agent: `writer-thesis`. Reviewer routing: `reviewer-deep`.
7. **`thesis/chapters/04_data_and_methodology.md` §4.4.5 Tabela 4.7 sc2egset CI cell** — the chapter cell already carries an inline `UNVERIFIED` flag noting that `icc.json` does not name the CI method explicitly and that the DEFEND document suggests a delta-method per `variance_icc_sc2egset.csv`. Recorded here as a known caveat carried forward by the writer at next Pass-2 touch. Severity: **LOW**. Future agent: `writer-thesis`. Reviewer routing: `reviewer-deep`.

### 10.5 Missing claims to add

The following claims are supported by current evidence but absent from drafted chapter prose. Each missing-claim row resolves to a future writing PR (per `planning/current_plan.md` §Hard constraints for execution: this PR is Category E docs-only; no chapter prose is drafted here).

1. **NEW `thesis/chapters/04_data_and_methodology.md` §4.5 — Phase 02 registry methodology.** Missing claim: the SC2EGSet provisional feature-family registry artifact emitted by PR #216 (CSV + MD, `validated_through = V-9`, manifest token `partial_coverage_v9_baseline`); the V-1..V-9 validation baseline with V-9 framed as "structural guard against future drift" (per registry MD verbatim); the deferred-dimension table (D2, D3, D4-in_game, D5-in_game, D6-full, D8, D9 — resolved at materialization step 02_01_02 via CROSS-02-01 post-materialization audit); the non-supersession statement. Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.{csv,md}`; `thesis/pass2_evidence/notebook_regeneration_manifest.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/research_log.md`; `reports/specs/02_00_feature_input_contract.md`; `reports/specs/02_01_leakage_audit_protocol.md`; `reports/specs/02_03_temporal_feature_audit_protocol.md`. Severity: **MEDIUM** (HIGH for the per-dataset asymmetry sub-claim per §10.4 row 4). Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`. Routes to audit §9.1 PR.
2. **NEW `thesis/chapters/04_data_and_methodology.md` §4.3.x — SC2 tracker eligibility subsection.** Missing claim: 15-row eligibility CSV (5 `eligible_for_phase02_now` + 7 `eligible_with_caveat` + 3 `blocked_until_additional_validation`); GATE-14A6 `narrowed` outcome; tracker features ARE NEVER pre-game (Invariant I3 / Amendment 2). Evidence: `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`; `thesis/pass2_evidence/phase02_readiness_hardening.md`; `thesis/pass2_evidence/methodology_risk_register.md` RISK-21. Severity: **HIGH**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`. Routes to audit §9.2 PR. This row also resolves the C2 stale claim recorded in §10.2 row 1.
3. **NEW §4.5 (cross-cut with row 1) — `partial_coverage_v9_baseline` manifest row.** Missing claim: explicit reference to the manifest token introduced by PR #216 in `thesis/pass2_evidence/notebook_regeneration_manifest.md`. Evidence: manifest file (vocab docstring + row). Severity: **MEDIUM**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`.
4. **NEW §4.5 (cross-cut with row 1) — deferred-dimension table.** Missing claim: explicit enumeration of CROSS-02-03 dimensions deferred to materialization (D2, D3, D4-in_game, D5-in_game, D6-full, D8, D9), with the non-supersession statement attached. Evidence: registry MD §"What V-1..V-9 do NOT enforce — deferred dimensions"; `reports/specs/02_03_temporal_feature_audit_protocol.md` §4 table. Severity: **MEDIUM**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`.
5. **NEW §4.5 (cross-cut with row 1) — non-supersession disclaimer.** Missing claim: the registry's `validated_through = V-9` status does NOT excuse a materialized column from CROSS-02-01-v1.0.1 post-materialization audit. Evidence: registry MD §"Non-supersession of the post-materialization audit"; CROSS-02-01-v1.0.1 spec text. Severity: **MEDIUM**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`.
6. **NEW or revised section — CROSS-02-01 mandatory.** Missing claim: every Phase-02-fed materialized column passes the CROSS-02-01-v1.0.1 leakage audit before entering modelling. Evidence: `reports/specs/02_01_leakage_audit_protocol.md`. Severity: **MEDIUM**. Future agent: `planner-science-then-writer-thesis`. Reviewer routing: `reviewer-adversarial`. Routes to audit §9.4 PR.
7. **`thesis/chapters/05_experiments_and_results.md` placeholder subsections.** All sections are HTML-comment placeholders (`<!-- BLOCKED -->`); no drafted prose. Per Q1 resolution (recorded in `planning/current_plan.md` §Open questions): record as missing-claim, severity = LOW, future agent = `defer`. May be activated only after Phase 03 evidence exists. Reviewer routing: `reviewer` (when activated; n/a until then).
8. **`thesis/chapters/06_discussion.md` placeholder subsections.** Same status as row 7. §6.5 Threats to validity is DRAFTABLE per `thesis/WRITING_STATUS.md` (RISK-01..26 in the methodology risk register support a partial draft now) but the Q1 resolution treats the whole chapter as defer until Chapter 5 has content. Severity = LOW. Future agent = `defer`. Reviewer routing: `reviewer`.
9. **`thesis/chapters/07_conclusions.md` placeholder subsections.** Same status as row 7. §7.3 Future work is DRAFTABLE per WRITING_STATUS (idea accumulation can begin now) but Q1 resolution = defer. Severity = LOW. Future agent = `defer`. Reviewer routing: `reviewer`.

### 10.6 Exact proposed edits

The following table is the per-row routing artifact produced from §10.2–§10.5. Every row has a verified `evidence_source` path that exists on disk (spot-checked against `aa718f88`). Severity and routing follow the conventions stated in `planning/current_plan.md` §T07. `agent_to_use` values are: `writer-thesis`, `planner-science-then-writer-thesis`, or `defer`. `reviewer_routing` values are: `reviewer`, `reviewer-deep`, or `reviewer-adversarial`. This table is the input that §11 (Draft correction backlog) and §12 (Writing agent task queue) consume.

| thesis_file | section | issue_type | severity | current_claim | corrected_claim | evidence_source | reviewer_required | writing_agent_handoff_note |
|---|---|---|---|---|---|---|---|---|
| `thesis/chapters/04_data_and_methodology.md` | §4.3.2 (line ~331) | stale | HIGH | "Do czasu wykonania odrębnej walidacji semantycznej Step 01_03_05 — która nie została zrealizowana w obecnej iteracji — cechy oparte bezpośrednio na `tracker_events_raw` nie są traktowane jako w pełni zwalidowany komponent..." | Rewrite paragraph: Step 01_03_05 IS complete (`completed_at: 2026-05-05`); GATE-14A6 outcome is `narrowed` (not closed); 5 `eligible_for_phase02_now` + 7 `eligible_with_caveat` + 3 `blocked_until_additional_validation`; tracker features are NEVER pre-game (I3, Amendment 2). | `src/rts_predict/games/sc2/datasets/sc2egset/reports/STEP_STATUS.yaml`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`; `thesis/pass2_evidence/phase02_readiness_hardening.md` | reviewer-adversarial | Do NOT claim Step closure (F4). Do NOT claim "leakage-free" (F7). MUST cite GATE-14A6 outcome verbatim as `narrowed`. MUST state tracker features are never pre-game. Pair with row 4 (NEW §4.3.x) — they resolve together. |
| `thesis/chapters/02_theoretical_background.md` ⨯ `thesis/chapters/03_related_work.md` | §2.5.5 (line ~179) ⨯ §3.2.4 (line ~77) ⨯ §3.5 (line ~189) | stale | MEDIUM | §2.5.5: "wersja HuggingFace v8.0, cutoff 2025-12-31"; §3.2.4: "v9.0, cutoff 2025-09-30; v8.0 planowana z cutoff 2025-12-31"; §3.5: "v9.0 (cutoff 2026-03-31, weryfikacja 2026-04-26)" | Pick one canonical (version, cutoff, verification-date) triple at Pass-2 finalization; harmonize all three loci; remove the §3.2.4 internal contradiction first. | `thesis/pass2_evidence/literature_verification_log.md`; `thesis/WRITING_STATUS.md` (chap2 / chap3 rows); WebFetch HF dataset card (Pass-2 only) | reviewer-deep | Do NOT introduce a new version citation without WebFetch verification of HuggingFace commit log. Resolve §3.2.4's internal contradiction first (`v9.0` paired with a `v7.0`-era cutoff is incoherent). |
| `thesis/chapters/04_data_and_methodology.md` | §4.1.4 (line ~212) | missing-caveat | MEDIUM | "phase06_interface_aoestats.csv (136 wierszy) niesie tag dla aoestats" (interface-CSV row-count framing) | Clarify: 137 rows total (136 data + 1 header) and aoestats scope is implicit-via-spec-§0 + R02, not tag-carried (0 of 137 `[POP:]` tags). | §1.1 and §4.1.4 of this audit; `thesis/pass2_evidence/cross_dataset_comparability_matrix.md` | reviewer-deep | Do NOT delete the existing `[REVIEW]` flag at line 211 without verifying audit's reconciled framing. |
| NEW `thesis/chapters/04_data_and_methodology.md` §4.3.x | absent (NEW subsection) | missing-claim | HIGH | absent | Draft a new subsection enumerating: 15-row eligibility CSV breakdown (5 + 7 + 3); GATE-14A6 `narrowed` outcome; per-row `eligibility_scope` / `caveat` constraints; Invariant I3 / Amendment 2 (tracker features never pre-game). | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`; `thesis/pass2_evidence/phase02_readiness_hardening.md`; `thesis/pass2_evidence/methodology_risk_register.md` | reviewer-adversarial | Do NOT promote any of the 3 `blocked_until_additional_validation` rows. Do NOT claim closure of GATE-14A6. Cite the eligibility CSV row count explicitly. Pair with row 1 (§4.3.2 stale rewrite). |
| NEW `thesis/chapters/04_data_and_methodology.md` §4.5 | absent (NEW subsection) | missing-claim | MEDIUM (HIGH for per-dataset asymmetry sub-claim) | absent | Draft a new subsection: SC2EGSet provisional feature-family registry artifact (CSV + MD, `validated_through = V-9`, manifest token `partial_coverage_v9_baseline`); V-1..V-9 validation baseline with V-9 as "structural guard against future drift"; deferred-dimension table (D2, D3, D4-in_game, D5-in_game, D6-full, D8, D9); non-supersession statement; per-dataset Phase 02 asymmetry (aoestats + aoe2companion = ROADMAP-stub only); CROSS-02-01-v1.0.1 mandatory. | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.csv`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.md`; `thesis/pass2_evidence/notebook_regeneration_manifest.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/research_log.md`; `reports/specs/02_00_feature_input_contract.md`; `reports/specs/02_01_leakage_audit_protocol.md`; `reports/specs/02_03_temporal_feature_audit_protocol.md` | reviewer-adversarial | Do NOT claim Step 02_01_01 closure (F4). Do NOT claim final feature catalog (F3). Do NOT claim leakage-free (F7). Do NOT claim aoestats/aoe2companion parity (F5). MUST cite the registry as provisional verbatim. MUST attach the non-supersession statement. |
| `thesis/chapters/05_experiments_and_results.md` | §5.1.1–§5.3.3 (all placeholders) | missing-claim | LOW | absent (HTML-comment placeholders only) | Defer until Phase 03 evidence exists. No edit in any PR scoped to PR #216 evidence. | `planning/current_plan.md` §Open questions Q1 (RESOLVED 2026-05-17); `thesis/WRITING_STATUS.md` | reviewer | Do NOT activate. Q1 resolution: agent_to_use = defer. |
| `thesis/chapters/06_discussion.md` | §6.1–§6.5 (all placeholders) | missing-claim | LOW | absent (HTML-comment placeholders only) | Defer until Chapter 5 has content. §6.5 Threats to validity DRAFTABLE per WRITING_STATUS but Q1 defers the whole chapter. | `planning/current_plan.md` §Open questions Q1 (RESOLVED 2026-05-17); `thesis/WRITING_STATUS.md`; `thesis/pass2_evidence/methodology_risk_register.md` | reviewer | Do NOT activate. Q1 resolution: agent_to_use = defer. |
| `thesis/chapters/07_conclusions.md` | §7.1–§7.3 (all placeholders) | missing-claim | LOW | absent (HTML-comment placeholders only) | Defer until Chapters 5+6 have content. §7.3 Future work DRAFTABLE per WRITING_STATUS but Q1 defers the whole chapter. | `planning/current_plan.md` §Open questions Q1 (RESOLVED 2026-05-17); `thesis/WRITING_STATUS.md` | reviewer | Do NOT activate. Q1 resolution: agent_to_use = defer. |

---

## 11. Draft correction backlog

This table is the actionable consequence of §10 — every row in §10.2 / §10.4 / §10.5 (excluding the C1 safe-and-supported rows of §10.1 and the explicitly empty C3 register of §10.3) becomes one backlog row here. Every row routes to a **future writing PR**: no row authorizes editing chapter prose in this PR (PR #217 is Category E docs-only per `planning/current_plan.md`). AoE2 rows MUST preserve source-label discipline (`aoestats` is Tier 4 with unverified queue semantics; `aoe2companion ID 6` is `rm_1v1` ranked candidate; `aoe2companion ID 18` is `qp_rm_1v1` quickplay/matchmaking-derived; combined `ID 6 + ID 18` is mixed-mode, not ranked-only).

Allowed `issue_type` values: `stale`, `overclaim`, `missing-caveat`, `missing-claim`, `source-label-violation`. (§11 contains zero `overclaim` rows because §10.3 found `C3 overclaim count: 0`; zero `source-label-violation` rows because T05/T06 confirmed AoE2 source-label discipline currently holds across drafted chapters.)

Table 11.A — Draft correction backlog:

| thesis_file | section | issue_type | severity | current_claim | corrected_claim | evidence_source | reviewer_required | writing_agent_handoff_note |
|---|---|---|---|---|---|---|---|---|
| `thesis/chapters/04_data_and_methodology.md` | §4.3.2 (line ~331) | stale | HIGH | Step 01_03_05 tracker semantic validation "nie została zrealizowana w obecnej iteracji" (chapter prose asserts the validation step is not yet performed). | Step 01_03_05 completed 2026-05-05; GATE-14A6 outcome is `narrowed`, not `closed`; 12 planned-yes tracker families are ready under per-row `eligibility_scope` / `caveat`, and 3 remain `blocked_until_additional_validation`; tracker-derived features are never pre-game (Invariant I3 / Amendment 2 per PR #208). | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`; `thesis/pass2_evidence/phase02_readiness_hardening.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/STEP_STATUS.yaml` | reviewer-adversarial | Do not say the full tracker scope is closed; use the `narrowed` framing. Pair with backlog row 4 (NEW §4.3.x) — both resolve together. Do not promote any `blocked_until_additional_validation` row. |
| `thesis/chapters/02_theoretical_background.md`, `thesis/chapters/03_related_work.md` | §2.5.5 (line ~179) ⨯ §3.2.4 (line ~77) ⨯ §3.5 (line ~189) | stale | MEDIUM | Cross-locus EsportsBench version-and-cutoff drift: §2.5.5 cites `v8.0, cutoff 2025-12-31`; §3.2.4 cites `v9.0, cutoff 2025-09-30; v8.0 planowana z cutoff 2025-12-31` (internally inconsistent); §3.5 cites `v9.0 (cutoff 2026-03-31, verification 2026-04-26)`. | Future writing PR must harmonize the version / date / cutoff story across all three loci after Pass-2 source verification. Resolve §3.2.4's internal contradiction first. | `thesis/pass2_evidence/literature_verification_log.md`; `thesis/WRITING_STATUS.md` | reviewer-deep | Do not invent the current EsportsBench version; verify with the upstream source (HuggingFace dataset card commit log) before finalizing prose. Do not introduce a new version citation without WebFetch verification. |
| `thesis/chapters/04_data_and_methodology.md` | §4.1.4 (line ~212) | missing-caveat | MEDIUM | "phase06_interface_aoestats.csv (136 wierszy) niesie tag dla aoestats" — count framing collides with the audit's reported 137-row count for the same file. | Align with audit value: 137 rows total (136 data + 1 header), or explicitly explain the header-vs-data framing. Acknowledge that aoestats `[POP:]` scope is implicit-via-spec-§0 + R02, not tag-carried (0 of 137 `[POP:]` tags). | `thesis/pass2_evidence/phase01_phase02_writing_readiness_audit.md` §1.1 + §4.1.4; `thesis/pass2_evidence/cross_dataset_comparability_matrix.md` | reviewer-deep | Do not delete the existing `[REVIEW]` chapter flag at line 211 without verifying audit's reconciled framing first. |
| NEW `thesis/chapters/04_data_and_methodology.md` §4.3.x (subsection does not yet exist) | NEW | missing-claim | MEDIUM | absent | Add a new SC2 tracker eligibility subsection: enumerate the 15-row eligibility CSV breakdown (5 `eligible_for_phase02_now` + 7 `eligible_with_caveat` + 3 `blocked_until_additional_validation`); state GATE-14A6 = `narrowed`; describe per-row `eligibility_scope` and `caveat`; carry the "no tracker-derived pre-game features" invariant explicitly. | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`; `thesis/pass2_evidence/phase02_readiness_hardening.md`; `thesis/pass2_evidence/methodology_risk_register.md` | reviewer-adversarial | Requires `planner-science-then-writer-thesis`; do not let `writer-thesis` invent feature semantics. Do not claim closure of GATE-14A6. |
| NEW `thesis/chapters/04_data_and_methodology.md` §4.5 (subsection does not yet exist) | NEW | missing-claim | MEDIUM (HIGH for per-dataset asymmetry sub-claim) | absent | Add a Phase 02 registry methodology subsection covering: SC2EGSet provisional feature-family registry artifact (PR #216); `validated_through = V-9`; manifest token `partial_coverage_v9_baseline`; non-supersession of CROSS-02-01-v1.0.1 post-materialization audit; deferred-dimension table (D2, D3, D4-in_game, D5-in_game, D6-full, D8, D9); per-dataset Phase 02 asymmetry (aoestats + aoe2companion = ROADMAP-stub only). | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.csv`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.md`; `thesis/pass2_evidence/notebook_regeneration_manifest.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/research_log.md`; `reports/specs/02_01_leakage_audit_protocol.md`; `reports/specs/02_03_temporal_feature_audit_protocol.md` | reviewer-adversarial | Requires `planner-science-then-writer-thesis`; do not claim Step 02_01_01 closure, final feature catalog, leakage-free materialized features, model-ready matrix, or aoestats/aoe2companion parity. Must attach the non-supersession statement verbatim. |
| `thesis/chapters/05_experiments_and_results.md` | §5.1.1–§5.3.3 (all placeholders) | missing-claim | LOW | placeholder only (HTML-comment subsections; no drafted prose). | Defer until Phase 03 / Phase 04 / Phase 05 experiments exist; do not draft any model-performance claim from current evidence. | `thesis/WRITING_STATUS.md`; `planning/current_plan.md` §Open questions Q1 (RESOLVED 2026-05-17) | reviewer | `agent_to_use = defer`. Do not draft results without model artifacts. |
| `thesis/chapters/06_discussion.md` | §6.1–§6.5 (all placeholders) | missing-claim | LOW | placeholder only (HTML-comment subsections; no drafted prose). §6.5 Threats to validity is DRAFTABLE per WRITING_STATUS but Q1 defers the whole chapter. | Defer until Chapter 5 experimental results exist; no result interpretation until model artifacts land. | `thesis/WRITING_STATUS.md`; `planning/current_plan.md` §Open questions Q1 (RESOLVED 2026-05-17); `thesis/pass2_evidence/methodology_risk_register.md` | reviewer | `agent_to_use = defer`. Do not infer results from registry artifact alone. |
| `thesis/chapters/07_conclusions.md` | §7.1–§7.3 (all placeholders) | missing-claim | LOW | placeholder only (HTML-comment subsections; no drafted prose). §7.3 Future work is DRAFTABLE per WRITING_STATUS but Q1 defers the whole chapter. | Defer until results (Chapter 5) and discussion (Chapter 6) are complete. | `thesis/WRITING_STATUS.md`; `planning/current_plan.md` §Open questions Q1 (RESOLVED 2026-05-17) | reviewer | `agent_to_use = defer`. Do not conclude tabular-vs-GNN or cross-game generalizability. |

---

## 12. Writing agent task queue

Each task in this queue maps one or more rows of §11 (Draft correction backlog) to a future writing PR. Every task names the writing agent that handles it, the precise target file/section, the allowed evidence sources, the forbidden claims (linked to §8 Forbidden Claims F1–F18 wherever applicable), the reviewer routing, and the expected deliverable. This queue is the **authoritative routing artifact** for any future Category-F drafting PR scoped against post-PR #216 evidence; any future writing prompt MUST cite the relevant TQ row verbatim and configure the writing agent with the explicit `forbidden_claims` cell.

Table 12.A — Writing agent task queue:

| task_id | agent_to_use | target_thesis_file_or_section | allowed_evidence_sources | forbidden_claims | reviewer_routing | expected_output |
|---|---|---|---|---|---|---|
| TQ-01 | writer-thesis | `thesis/chapters/04_data_and_methodology.md` §4.3.2 (line ~331) | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`; `thesis/pass2_evidence/phase02_readiness_hardening.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/STEP_STATUS.yaml` | F1 (RQ1 closure); F3 (final feature catalog); F4 (Step / Phase closure overclaim); F7 (leakage-free materialized features); F8 (pre-game tracker features — tracker is never pre-game); F15 (Friedman at N=2); no "closed" tracker scope; no promotion of any `blocked_until_additional_validation` row | reviewer-adversarial | Local-paragraph rewrite of §4.3.2 reflecting GATE-14A6 `narrowed` framing, Step 01_03_05 `completed_at: 2026-05-05`, and the 5 + 7 + 3 eligibility breakdown. Coordinate with TQ-02 (single PR or paired PRs). |
| TQ-02 | planner-science-then-writer-thesis | NEW `thesis/chapters/04_data_and_methodology.md` §4.3.x SC2 tracker eligibility subsection (does not yet exist) | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/01_03_05_tracker_events_semantic_validation.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/01_exploration/03_profiling/tracker_events_feature_eligibility.csv`; `thesis/pass2_evidence/phase02_readiness_hardening.md`; `thesis/pass2_evidence/methodology_risk_register.md`; `.claude/scientific-invariants.md` (I3 + Amendment 2) | F3 (final feature catalog); F4 (Step / Phase closure); F7 (leakage-free); F8 (tracker pre-game); no full tracker-scope closure; no tracker-derived pre-game features; no promotion of `blocked_until_additional_validation` rows | reviewer-adversarial | New §4.3.x subsection grounded in the tracker semantic validation MD + eligibility CSV; cites GATE-14A6 `narrowed`, 5 + 7 + 3 row split, per-row scope/caveat, and Invariant I3 (Amendment 2). Methodology design must precede prose. |
| TQ-03 | planner-science-then-writer-thesis | NEW `thesis/chapters/04_data_and_methodology.md` §4.5 Phase 02 registry methodology subsection (does not yet exist) | `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.csv`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/artifacts/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_01_feature_family_registry.md`; `thesis/pass2_evidence/notebook_regeneration_manifest.md`; `src/rts_predict/games/sc2/datasets/sc2egset/reports/research_log.md`; `reports/specs/02_00_feature_input_contract.md`; `reports/specs/02_01_leakage_audit_protocol.md`; `reports/specs/02_03_temporal_feature_audit_protocol.md` | F2 (tabular vs GNN); F3 (final feature catalog); F4 (Step 02_01_01 / Phase 02 closure); F5 (aoestats / aoe2companion parity with SC2EGSet); F7 (leakage-free materialized features); no model-ready matrix claim; no "complete" Phase 02 framing | reviewer-adversarial | New §4.5 prose citing PR #216 as provisional (`validated_through = V-9`), manifest token `partial_coverage_v9_baseline`, deferred-dimension table, non-supersession of CROSS-02-01-v1.0.1, per-dataset Phase 02 asymmetry. Methodology design must precede prose. |
| TQ-04 | writer-thesis | `thesis/chapters/02_theoretical_background.md` §2.5.5 + `thesis/chapters/03_related_work.md` §3.2.4 + `thesis/chapters/03_related_work.md` §3.5 | `thesis/pass2_evidence/literature_verification_log.md`; `thesis/WRITING_STATUS.md` | No unverified EsportsBench version, cutoff, or verification date; no benchmark-coverage claim beyond what the upstream dataset card asserts at the chosen version | reviewer-deep | Harmonized (version, cutoff, verification-date) triple across all three loci; §3.2.4 internal contradiction removed first. WebFetch of HuggingFace dataset-card commit log required during Pass-2 verification. |
| TQ-05 | writer-thesis | `thesis/chapters/04_data_and_methodology.md` §4.1.4 (line ~212) | `thesis/pass2_evidence/phase01_phase02_writing_readiness_audit.md` (audit §1.1 + §4.1.4); `thesis/pass2_evidence/cross_dataset_comparability_matrix.md` | F5 (cross-game / cross-dataset parity overclaim); no unverified `phase06_interface_aoestats.csv` row count; no `[POP:]` tag-presence claim for aoestats (scope is implicit-via-spec-§0 + R02) | reviewer-deep | §4.1.4 row-count framing corrected to 137 rows (or explicit header-vs-data clarification) and `[POP:]` aoestats scope reworded to implicit-via-spec rather than tag-carried. |
| TQ-06 | defer | `thesis/chapters/05_experiments_and_results.md` | `thesis/WRITING_STATUS.md`; `planning/current_plan.md` §Open questions Q1 | F1 (RQ1 closure); F2 (tabular vs GNN); F3 (final feature catalog); F5 (cross-game generalizability); F11 (model performance / accuracy / Brier / log-loss); all model-performance claims; all tabular-vs-GNN conclusions | reviewer | No writing in any PR scoped to PR #216 evidence. Activate only after Phase 03+ model artifacts exist on disk. |
| TQ-07 | defer | `thesis/chapters/06_discussion.md` | `thesis/WRITING_STATUS.md`; `planning/current_plan.md` §Open questions Q1; `thesis/pass2_evidence/methodology_risk_register.md` | Any result interpretation without model artifacts; F2 / F5 / F11 as for TQ-06; no causal claim about cross-game differences (§1.3 RQ3 four-confound disclaimer must remain binding) | reviewer | No writing until Chapter 5 has evidence. §6.5 Threats to validity may be partial-drafted in a separate optional PR after the Q1 deferral is explicitly lifted by user decision. |
| TQ-08 | defer | `thesis/chapters/07_conclusions.md` | `thesis/WRITING_STATUS.md`; `planning/current_plan.md` §Open questions Q1 | Final conclusions; F1 (RQ1 closure); F2 (tabular vs GNN); F5 (cross-game generalizability); F11 (model performance); no Chapter-6-anticipating discussion claims | reviewer | No writing until results (Chapter 5) and discussion (Chapter 6) are complete. §7.3 Future work may be partial-drafted in a separate optional PR after the Q1 deferral is explicitly lifted. |
