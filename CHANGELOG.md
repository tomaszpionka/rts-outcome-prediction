...
- Removed the immortal exact-SHA pin on the SC2EGSet dataset `research_log.md`
  in `adjudicate_history_rating_reconstruction.py`
  (`EXPECTED_DATASET_RESEARCH_LOG_SHA256`, `_check_dataset_research_log_sha256`,
  the `dataset_research_log_sha256_mismatch` falsifier key, and the
  `EXPECTED_DATASET_RESEARCH_LOG_SHA256` line in the audit MD's §16 SHA
  provenance block). The dataset `research_log.md` is append-only mutable
  lineage; the original PR #245 pin was a category error that bound
  adjudicator pass/fail to a snapshot guaranteed to drift on every legitimate
  Category A append.

### Added

- Added `DATASET_RESEARCH_LOG_PR245_EVIDENCE_TOKENS`
  (3 verbatim PR #245-era anchor strings cited in this module's own docstring)
  and the append-only-safe falsifier
  `_check_dataset_research_log_evidence_present` — halts only on
  evidence loss / tampering (a required token absent), not on benign
  later appends. Wired into `run_rating_reconstruction_adjudication`
  under the new falsifier key `dataset_research_log_evidence_missing`.
- Added the provenance-only helper `compute_dataset_research_log_sha256`
  (returns the current SHA-256 for observability; never compared to a
  hardcoded expected value).
- Added regression tests in
  `tests/rts_predict/games/sc2/datasets/sc2egset/test_adjudicate_history_rating_reconstruction.py`
  (`TestDatasetResearchLogAppendOnlySafety`) covering: benign-append
  pass; PR #245 evidence-removal halt; independent load-bearing
  parametrization over all 3 tokens; missing-file halt; provenance
  helper digest validity and `NOT_FOUND` path.
- Refreshed two existing dataset-research-log tests to the new
  evidence-presence semantics (`test_dataset_research_log_evidence_fires_on_tampered`
  and `test_dataset_research_log_evidence_present_passes`).

### Fixed

- The full repo `pytest` suite passes again (2729 tests). The 7-test
  regression introduced by the PR #245 immortal pin is closed, unblocking
  PR #259 (`feat/sc2egset-02-01-03-five-family-materialization`), which
  legitimately appends a non-closure entry to the dataset `research_log.md`
  per its merged Layer-1 plan and will be rebased onto new master and
  re-gated after this chore PR merges.

### Notes

- The 45-entry `HELPER_TO_FALSIFIER_KEY` / `FALSIFIER_PRIORITY_CHAIN`
  invariant is preserved (one helper key was renamed, none removed).
- The PR #245 audit MD at
  ...
  remains byte-unchanged: it is a generated artifact and was historically
  accurate at PR #245 merge time. It will be regenerated by its canonical
  lineage when (and only when) the Q6 adjudicator is rerun by an
  authorised future PR.
- No edits to `STEP_STATUS.yaml`, `PIPELINE_SECTION_STATUS.yaml`,
  `PHASE_STATUS.yaml`, ROADMAP.md, the dataset `research_log.md`, the
  root `reports/research_log.md`, any spec/cleaning-layer YAML, any
  generated artifact, or any PR #259 file. Chore-class hygiene PR;
  patch bump only.
...
- Mirrored test file `tests/rts_predict/games/sc2/datasets/sc2egset/test_adjudicate_history_rating_reconstruction.py` — 315 tests, 99.54% branch coverage on the adjudicator module (exceeds the 95% project threshold).
- Sandbox jupytext-paired notebook at `sandbox/sc2/sc2egset/02_feature_engineering/01_pre_game_vs_in_game_boundary/02_01_03_history_rating_reconstruction_adjudication.{py,ipynb}` (py:percent canonical; outputs cleared; all logic imported from the adjudicator module).
- Binding adversarial-review nits N-1 through N-4 from PR #244 Round 1 implemented: N-1 (Aligulac BTL / Bradley-Terry / Neural BTL acknowledged via `excluded_methods_considered` field and MD §5.1 rejection paragraph; not selected as Q6 candidates); N-2 (raw-MMR-where-present hybrid acknowledged via `raw_mmr_hybrid_rejection` field and MD §5.2 rejection paragraph); N-3 (per-player history-depth probe — `player_history_all` has NO `player_id_worldwide` column per `DESCRIBE`, so the probe groups by `toon_id` with documented inline rationale citing the empirical schema constraint); N-4 (all probes are full-table scans with deterministic `ORDER BY` clauses where needed; no `LIMIT` without `ORDER BY`).
- Soft nits N-5 through N-10 incorporated where mechanically practical (N-7 inlined into PR #244 plan; N-9 `POST_GAME_TOKENS` imported from PR #242 `adjudicate_history_enriched_pre_game_source_layer` with word-boundary regex semantic refinement to avoid false-positive substring collisions with `rolling_win_rate`; N-10 MD §5.3 explicitly states Q6F is a legitimate verdict).

### Provenance / lineage
- Parent PR #242 (`f2a169ec...` CSV, `fdaa7d6d...` MD) and parent PR #243 (`29d39522...` CSV, `026deda3...` MD) byte-stable; SHAs pinned on every Q6 decision row.
- Q5 selected_policy = `sensitivity_indicator_co_registration` (PR #243) is BINDING and NOT re-adjudicated.
- Step 02_01_03 base adjudication Q1-Q4, Q7, Q8 (PR #242) ratified; not re-adjudicated.

### Not in this PR (preserved hard stops; future Layer-3+ PRs)
- NO feature value materialised, NO Parquet output, NO CROSS-02-01 post-materialization audit, NO `reports/artifacts/02_01_03/leakage_audit_sc2egset.{json,md}` file.
- NO status YAML mutation (`STEP_STATUS.yaml` / `PIPELINE_SECTION_STATUS.yaml` / `PHASE_STATUS.yaml`).
- NO `research_log.md` entry (per PR #242 / PR #243 adjudication-artifact precedent; closure-log deferred).
- NO `ROADMAP.md` edit, NO spec edits, NO cleaning-layer YAML edits.
- NO Step 02_01_04 start, NO Phase 03 start, NO baseline modelling.
- NO thesis / docs / .claude / data / AoE2 edits.
